...
//utiliza um joystick para determinar input
//agora com som!!!
//tempo entre cores diminui apos cada rodada
//bug de resetar arrumado
//bug de resetar arrumado denovo
//
...
int temp=240;   //old value was 144 
...
int melody[] = {
  ...
  //Based on the arrangement at https://musescore.com/minecraftvideosr/undertale_-_megalovania
  
  NOTE_D3,16,  NOTE_D3,16,  NOTE_D4,8, NOTE_A3,8, REST,16, NOTE_GS3,16,  REST,16, NOTE_G3,16, REST,16, NOTE_F3,8, NOTE_D3,16, NOTE_F3,16, NOTE_G3, 16
};
...
int sequencia[50]; //sequencia total de cores
  ...
  while (i<=indice) 
    {
      ...
      if(sequencia[i]==AZ) tone(BUZZER, NOTE_C3);
      if(sequencia[i]==VD) tone(BUZZER, NOTE_D3);
      if(sequencia[i]==AM) tone(BUZZER, NOTE_E3);
      if(sequencia[i]==VM) tone(BUZZER, NOTE_F3);
