...
void abertura() { // melodia de entrada e leds piscando
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    if (divider > 0) {
      ...
      noteDuration = (wholenote) / divider;   for(int i=0; i<4; i++) digitalWrite(cores[i],HIGH); 
    } else if (divider < 0) {
    ...
    for(int i=0; i<4; i++) digitalWrite(cores[i],LOW);
 ...
 //    pisca--;
  //if (pisca < 0) {pisca = 100;}
  //if ((pisca = 1)&&(vez==false))  { for(int i=0; i<4; i++) digitalWrite(cores[i],HIGH); vez=true;}
  //if ((pisca = 1) && (vez==true))  { for(int i=0; i<4; i++) digitalWrite(cores[i],LOW); vez=false;}
  }
